...
> ```python
> bif.config.destinationTable.largeTableMergeMethodEnabled # default False
> bif.config.destinationTable.largeTableMergeMethodSourceThresholdGB # default 0.2
> bif.config.destinationTable.largeTableMergeMethodDestinationThresholdGB # default 20
> ```
> However, I wouldn't recommend using this method. If you run into memory bottlenecks, partitioning can usually help.

### SCD 2

Slowly chaning dimension type 2 tracks the history of changes in a table, creating a new row each time an attribute changes. Here we can see an example of changes in `name`. 
The RowStart, RowEnd, CurrentRow, as well as `branch_history_SID` (identity column can be configured through `identityColumnPattern`) are generated by `bifabrik`.

![image](https://github.com/user-attachments/assets/9efdff63-6bf6-4dd0-b695-f5a29d15e2d8)
...
There are a few other configuration options you could use:
 ...
 - `scd2SoftDelete` - when set to `true`, destination table rows that are not found in the source will be end-dated. This may not work well for incremental loads, so it's `false` by default.
 - `scd2ExcludeColumns` - this is an empty array by default. When loading SCD 2 tables, any change in non-key columns will cause a new version to be inserted. However, this may not always be the intended behaviour. For example, if you add a `Batch_ID` or a timestamp to your source data each time you load it, these generated columns would count as a new version of the row even if the source data did not change. Use `scd2ExcludeColumns` to exclude selected columns from the historization.
 - `rowStartColumn` - customize the name of the RowStart column
 - `rowEndColumn` - customize the name of the RowEnd column
 - `currentRowColumn` - customize the name of the CurrentRow column
